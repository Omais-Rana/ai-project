...
@section('content')
    <div class="px-4 sm:px-6 lg:px-8">
        ...
        <!-- Chat Interface -->
        <div x-data="chatInterface()" x-init="init()"
            class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden"
            style="height: calc(100vh - 300px); min-height: 500px;">
            <!-- Messages Container -->
            <div x-ref="messagesContainer" class="h-full overflow-y-auto p-6 space-y-4" style="height: calc(100% - 80px);">
                <!-- Welcome Message -->
                ...
                <template x-for="(message, index) in messages" :key="index">
                    <div :class="message.role === 'user' ? 'flex justify-end' : 'flex justify-start'">
                        <div :class="message.role === 'user' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-900'"
                            ...
                            <div class="text-sm font-medium mb-1" x-text="message.role === 'user' ? 'You' : 'Assistant'">
                            </div>
                            <div class="text-sm" x-html="formatMarkdown(message.content)"></div>

                            <!-- Citations (only show if answer is document-related) -->
                            <template
                                x-if="message.citations && message.citations.length > 0 && !isNonDocumentAnswer(message.content)">
                                <div class="mt-3 pt-3 border-t border-gray-200">
                                    ...
                                    <template x-for="citation in message.citations" :key="citation.chunk_id">
                                        <div class="text-xs opacity-75">
                                            ...
                                            - Similarity: <span
                                                x-text="(citation.similarity * 100).toFixed(0) + '%'"></span>
                                        </div>
                                    </template>
                                </div>
                            </template>
                            ...
                            <template x-if="message.confidence_percentage && !isNonDocumentAnswer(message.content)">
                                <div class="mt-2">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium"
                                        :class="message.is_high_confidence ? 'bg-green-100 text-green-800' :
            ...
            <div class="border-t border-gray-200 p-4">
                <form @submit.prevent="sendMessage" class="flex space-x-2">
                    <input type="text" x-model="question" :disabled="loading"
                                ...
                                if (data.success && data.data.length > 0) {
                                    ...
                                    data.data.forEach(record => {
                                        ...
                                        this.messages.push({
                                            ...
                                            confidence_percentage: record.confidence_percentage || Math
